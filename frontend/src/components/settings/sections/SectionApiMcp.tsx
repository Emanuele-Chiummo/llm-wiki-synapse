...
export function SectionApiMcp() {
  ...
  const handleCopy = () => {
    if (!info) return;
    navigator.clipboard.writeText(buildClaudeDesktopSnippet(info)).then(() => {
      setCopied(true);
  ...
  const handleDismissGeneratedToken = () => { setGeneratedToken(null); };
  ...
  const handleCopyRemoteSnippet = () => {
    navigator.clipboard.writeText(buildRemoteMcpSnippet(remoteUrl)).then(() => {
  ...
  return (
    <div>
      ...
      {err ? (
        <p style={{ fontSize: 12, color: "var(--syn-red)", margin: "8px 0" }}>{t("settings.apiMcp.error")}</p>
      ) : info === null ? (
        <p style={{ fontSize: 12, color: "var(--syn-text-muted)", margin: "8px 0" }}>{t("settings.apiMcp.loading")}</p>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <div>
            ...
            <div style={{ padding: "10px 14px", background: "var(--syn-bg-soft)", border: "1px solid var(--syn-border)", borderRadius: 8, marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <span aria-hidden="true" style={{ width: 8, height: 8, borderRadius: "50%", background: tokenConfigured ? "var(--syn-green)" : "var(--syn-text-dim)", flexShrink: 0, display: "inline-block" }} />
                <span data-testid="mcp-token-posture" style={{ fontSize: 12, fontWeight: 600, color: tokenConfigured ? "var(--syn-green)" : "var(--syn-text-dim)" }}>
                  {t(tokenPostureKey)}
                </span>
              </div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button
                  ...
                  disabled={authBusy}
                  style={{ ...BTN_PRIMARY, opacity: authBusy ? 0.4 : 1, cursor: authBusy ? "not-allowed" : "pointer" }}
                >
                  {authBusy ? "…" : tokenConfigured ? t("settings.apiMcp.access.rotateToken") : t("settings.apiMcp.access.generateToken")}
                </button>
                {tokenConfigured && (
                  <button
                    ...
                    disabled={authBusy}
                    style={{ padding: "6px 14px", border: "1px solid color-mix(in srgb, var(--syn-red) 30%, transparent 70%)", borderRadius: 6, background: "transparent", color: "var(--syn-red)", fontSize: 12, cursor: authBusy ? "not-allowed" : "pointer", fontWeight: 500, opacity: authBusy ? 0.4 : 1 }}
                  >
            ...
            {generatedToken !== null && (
              <div style={{ padding: "12px 14px", background: "color-mix(in srgb, var(--syn-green) 8%, var(--syn-mix-base) 92%)", border: "1px solid color-mix(in srgb, var(--syn-green) 30%, var(--syn-mix-base) 70%)", borderRadius: 8, marginBottom: 10 }}>
                <p style={{ margin: "0 0 6px", fontSize: 12, fontWeight: 700, color: "var(--syn-green)" }}>
                  {t("settings.apiMcp.access.revealWarning")}
                </p>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span data-testid="mcp-generated-token" style={{ flex: 1, fontFamily: "monospace", fontSize: 12, color: "var(--syn-text)", padding: "6px 10px", background: "var(--syn-bg)", border: "1px solid color-mix(in srgb, var(--syn-green) 30%, var(--syn-mix-base) 70%)", borderRadius: 4, wordBreak: "break-all", userSelect: "all" }}>
                    {generatedToken}
                  </span>
                  <button data-testid="mcp-copy-generated-token-btn" onClick={handleCopyGeneratedToken} style={{ padding: "6px 12px", border: "1px solid color-mix(in srgb, var(--syn-green) 30%, var(--syn-mix-base) 70%)", borderRadius: 4, background: copiedGenToken ? "var(--syn-green)" : "transparent", color: copiedGenToken ? "#fff" : "var(--syn-green)", fontSize: 11, cursor: "pointer", flexShrink: 0, transition: "background 0.15s, color 0.15s" }}>
                    {copiedGenToken ? t("settings.apiMcp.copied") : t("common.copy")}
                  </button>
                </div>
                <button data-testid="mcp-dismiss-generated-token-btn" onClick={handleDismissGeneratedToken} style={{ marginTop: 8, padding: "4px 10px", border: "1px solid var(--syn-border)", borderRadius: 4, background: "transparent", color: "var(--syn-text-muted)", fontSize: 11, cursor: "pointer" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "10px 14px", background: "var(--syn-bg-soft)", border: `1px solid ${allowWithoutToken ? "color-mix(in srgb, var(--syn-amber) 30%, var(--syn-mix-base) 70%)" : "var(--syn-border)"}`, borderRadius: 8, opacity: authBusy ? 0.6 : 1, transition: "border-color 0.15s, opacity 0.15s" }}>
              <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", userSelect: "none", flex: 1 }}>
                <span style={{ position: "relative", display: "inline-block", width: 36, height: 20, flexShrink: 0, marginTop: 1 }}>
                  <input type="checkbox" role="switch" aria-label={t("settings.apiMcp.access.allowWithoutTokenLabel")} data-testid="mcp-allow-without-token" checked={allowWithoutToken} disabled={authBusy} onChange={() => { void handleAllowWithoutTokenToggle(); }} style={{ position: "absolute", opacity: 0, width: 0, height: 0 }} />
                  <span aria-hidden="true" style={{ display: "block", width: 36, height: 20, borderRadius: 10, background: allowWithoutToken ? "var(--syn-amber)" : "var(--syn-border)", border: `1px solid ${allowWithoutToken ? "var(--syn-amber)" : "var(--syn-border-subtle)"}`, transition: "background 0.15s, border-color 0.15s" }} />
                  <span aria-hidden="true" style={{ position: "absolute", top: 3, left: allowWithoutToken ? 19 : 3, width: 14, height: 14, borderRadius: "50%", background: allowWithoutToken ? "#fff" : "var(--syn-text-dim)", transition: "left 0.15s, background 0.15s" }} />
                </span>
                <div>
                  ...
                  <p data-testid="mcp-allow-without-token-caveat" style={{ margin: "4px 0 0", fontSize: 11, color: "var(--syn-amber)", lineHeight: 1.5 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: "var(--syn-bg-soft)", border: `1px solid ${remoteEnabled ? "var(--syn-accent)" : "var(--syn-border)"}`, borderRadius: 8, marginBottom: 10, opacity: toggleBusy ? 0.6 : 1, transition: "border-color 0.15s, opacity 0.15s" }}>
              <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: canEnableRemote ? "pointer" : "not-allowed", userSelect: "none", flex: 1 }} title={canEnableRemote ? undefined : t("settings.apiMcp.remote.noTokenNote")}>
                <span style={{ position: "relative", display: "inline-block", width: 36, height: 20, flexShrink: 0 }}>
                  <input type="checkbox" role="switch" aria-label={t("settings.apiMcp.remote.enabledLabel")} data-testid="mcp-remote-toggle" checked={remoteEnabled} disabled={!canEnableRemote || toggleBusy} onChange={() => { void handleRemoteToggle(); }} style={{ position: "absolute", opacity: 0, width: 0, height: 0 }} />
                  <span aria-hidden="true" style={{ display: "block", width: 36, height: 20, borderRadius: 10, background: remoteEnabled ? "var(--syn-accent)" : "var(--syn-border)", border: `1px solid ${remoteEnabled ? "var(--syn-accent)" : "var(--syn-border-subtle)"}`, transition: "background 0.15s, border-color 0.15s" }} />
                  <span aria-hidden="true" style={{ position: "absolute", top: 3, left: remoteEnabled ? 19 : 3, width: 14, height: 14, borderRadius: "50%", background: remoteEnabled ? "#fff" : "var(--syn-text-dim)", transition: "left 0.15s, background 0.15s" }} />
                </span>
                <div>
            ...
            {remoteEnabled && (
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 12 }}>
                <div>
                  <p style={{ margin: "0 0 6px", fontSize: 11, color: "var(--syn-text-muted)" }}>{t("settings.apiMcp.remote.urlLabel")}</p>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span data-testid="mcp-remote-url" style={{ flex: 1, fontFamily: "monospace", fontSize: 12, color: "var(--syn-accent)", padding: "5px 8px", background: "var(--syn-accent-soft)", border: "1px solid color-mix(in srgb, var(--syn-accent) 30%, var(--syn-mix-base) 70%)", borderRadius: 4, wordBreak: "break-all" }}>
                      {remoteUrl}
                    </span>
                    <button data-testid="mcp-remote-url-copy" onClick={handleCopyRemoteUrl} style={{ padding: "5px 10px", border: "1px solid var(--syn-border)", borderRadius: 4, background: copiedRemote ? "color-mix(in srgb, var(--syn-green) 8%, var(--syn-mix-base) 92%)" : "transparent", color: copiedRemote ? "var(--syn-green)" : "var(--syn-text-muted)", fontSize: 11, cursor: "pointer", flexShrink: 0, transition: "background 0.15s, color 0.15s" }}>
                ...
                <div>
                  <p style={{ margin: "0 0 6px", fontSize: 11, color: "var(--syn-text-muted)" }}>{t("settings.apiMcp.remote.snippetLabel")}</p>
                  <div data-testid="mcp-remote-snippet" style={{ fontFamily: "monospace", fontSize: 11, background: "var(--syn-surface-sunken)", border: "1px solid var(--syn-border)", borderRadius: 6, padding: "10px 12px", color: "var(--syn-text-muted)", whiteSpace: "pre", overflowX: "auto", marginBottom: 6 }}>
                    {buildRemoteMcpSnippet(remoteUrl)}
                  </div>
                  <button data-testid="mcp-remote-snippet-copy" onClick={handleCopyRemoteSnippet} style={{ padding: "5px 12px", border: "1px solid var(--syn-border)", borderRadius: 6, background: copiedRemoteSnippet ? "color-mix(in srgb, var(--syn-green) 8%, var(--syn-mix-base) 92%)" : "transparent", color: copiedRemoteSnippet ? "var(--syn-green)" : "var(--syn-text-muted)", fontSize: 11, cursor: "pointer", transition: "background 0.15s, color 0.15s" }}>
          ...
          <div>
            ...
            <div style={{ marginTop: 14 }}>
              <div data-testid="mcp-snippet" style={{ fontFamily: "monospace", fontSize: 11, background: "var(--syn-surface-sunken)", border: "1px solid var(--syn-border)", borderRadius: 6, padding: "10px 12px", color: "var(--syn-text-muted)", whiteSpace: "pre", overflowX: "auto", marginBottom: 8 }}>
                {buildClaudeDesktopSnippet(info)}
              </div>
              <button onClick={handleCopy} data-testid="mcp-copy-btn" style={{ padding: "5px 12px", border: "1px solid var(--syn-border)", borderRadius: 6, background: copied ? "color-mix(in srgb, var(--syn-green) 8%, var(--syn-mix-base) 92%)" : "transparent", color: copied ? "var(--syn-green)" : "var(--syn-text-muted)", fontSize: 11, cursor: "pointer", transition: "background 0.15s, color 0.15s" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {info.tools.map((tool) => {
                ...
                const truncated = firstSentence.length > 80 ? firstSentence.slice(0, 79) + "…" : firstSentence;
                return (
                  <div key={tool.name} data-testid={`mcp-tool-row-${tool.name}`} className="settings-mcp-tool-row" style={{ display: "grid", gridTemplateColumns: "140px 1fr auto", gap: 10, alignItems: "center", padding: "8px 12px", background: "var(--syn-bg-soft)", border: "1px solid var(--syn-border)", borderRadius: 6 }}>
                    <span data-testid={`mcp-tool-name-${tool.name}`} style={{ fontFamily: "monospace", fontSize: 12, color: "var(--syn-text)", fontWeight: 600 }}>{tool.name}</span>
                    <span style={{ fontSize: 12, color: "var(--syn-text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{truncated}</span>
                    <span data-testid={`mcp-tool-params-${tool.name}`} data-param-count={paramCount} style={{ fontSize: 11, color: "var(--syn-text-dim)", whiteSpace: "nowrap" }}>
          ...
          {/* ── CLI Subscription Auth — ADR-0043 §2.6 ── */}
          <SectionCliAuth />
        </div>
      )}
    </div>
