...
export function SectionCosts() {
  ...
  const renderDayBars = (days: CostsSummary["by_day"]) => {
    ...
    return (
      <svg width={totalW} height={H + 16} data-testid="costs-day-chart" aria-label={t("settings.costs.byDay")} role="img" style={{ display: "block", overflow: "visible" }}>
        {days.map((d, i) => {
          ...
          return (
            <g key={d.date}>
              ...
              <rect x={x} y={y} width={BAR_W} height={barH} fill="var(--syn-accent)" opacity={0.8} rx={1} />
            </g>
          );
        })}
      </svg>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
        ...
        <input type="month" data-testid="costs-month-selector" value={month} onChange={handleMonthChange} style={{ ...INPUT_STYLE, width: 160 }} />
        <button data-testid="costs-refresh-btn" onClick={() => { void load(month); }} disabled={loading} style={{ ...BTN_PRIMARY, opacity: loading ? 0.4 : 1, cursor: loading ? "not-allowed" : "pointer" }}>
          ...
          {data.by_provider.length > 0 && (
            <div style={COST_CARD}>
              ...
              {data.by_provider_note && (
                <p style={{ fontSize: 11, color: "var(--syn-text-dim)", margin: "0 0 8px", lineHeight: 1.5 }}>{data.by_provider_note}</p>
              )}
