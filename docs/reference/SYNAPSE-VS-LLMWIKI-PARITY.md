...
| typeAffinity signal semantics (cross-type bonus) | Audit: `TYPE_AFFINITY` matrix awards CROSS-type pairs (entity↔concept=1.2), not same-type (01-AUDIT §F4 inaccuracy 1) | `backend/app/graph/engine.py` — uses `same_type` signal (1 if same type, 0 otherwise). Different approach: binary same-type flag, not a lookup matrix | 🟡 | Synapse's `same_type` signal gives +1.0 to same-type pairs. llm_wiki's `TYPE_AFFINITY` matrix penalizes same-type (0.5–0.8) and rewards cross-type (1.2). The audit confirms llm_wiki's README description is WRONG; the CODE awards cross-type. Synapse uses a simpler binary signal that has the opposite directional effect for entity↔entity pairs. For strict behavioral parity, Synapse should adopt an affinity lookup matrix (per the actual llm_wiki code, not the README). This affects the graph topology. | P1 | [BE] |
...
| Graph insights — surprising connections | `graph-insights.ts:31-102` — composite score, threshold ≥3, excludes index/log/overview nodes | Not implemented. | ❌ | Graph insights panel (surprising connections, knowledge gaps, bridge nodes) is absent from Synapse's GraphViewer. This is a key user-visible F4 sub-feature. | P1 | [BE]/[FE] |
| Graph insights — knowledge gaps (isolated / sparse community / bridge nodes) | `graph-insights.ts:141-179` — isolated (deg≤1), sparse (cohesion<0.15 & ≥3 nodes), bridge (≥3 communities) | Absent. | ❌ | Same as above — part of the insights panel gap. | P1 | [BE]/[FE] |
| Graph insights dismissable + click-to-highlight | `graph-view.tsx:800-805` — dismissable; `:1516` — click highlights node in graph | Absent. | ❌ | Same gap. | P1 | [FE] |
| Deep Research button from knowledge gap | `graph-view.tsx:1577-1588` — gap insight has "Deep Research" button that reads overview+purpose | Absent (no insights panel). | ❌ | Same gap. | P1 | [FE] |
...
| Feature / sub-behavior | llm_wiki behavior (audit ref) | Synapse current state (code ref) | Verdict | Gap (exact, actionable) | Priority | Owner |
|---|---|---|---|---|---|---|
| LaTeX → Unicode conversion (100+ symbols) | `latex-to-unicode.ts` — 168 entries (~145 distinct glyphs); applied outside math blocks | No corresponding file or reference found in `frontend/src/` | ❌ | LaTeX → Unicode conversion is entirely absent from Synapse's frontend. Math content from LLMs will display raw LaTeX (`$\alpha$`) instead of Unicode (α). | P1 | [FE] |
| KaTeX rendering for display math | `chat-message.tsx:5-7` — imports `remarkMath`+`rehypeKatex`+CSS; applied to ReactMarkdown | `frontend/src/components/chat/MarkdownView.tsx` — uses ReactMarkdown/remark-gfm; no `remarkMath`/`rehypeKatex` import visible | ❌ | KaTeX rendering absent. Math blocks (`$$…$$`) will not render as formatted formulas. | P1 | [FE] |
| Auto-wrap bare LaTeX environments with $$ | `chat-message.tsx:1123-1126` — wraps `\begin{…}\end{…}` bare with `$$` | Absent. | ❌ | Same gap. | P1 | [FE] |
...
| K3 — index.md catalogue | Updated at every ingest (content catalog) | `backend/app/wiki/index.py:update_index` — called from `write_wiki_page` | ✅ | | — | — |
...
| K5 — `[[wikilink]]` syntax with dedicated parser | `wiki-graph.ts` / `graph-relevance.ts` — regex parser | `backend/app/wiki/links.py:parse_wikilinks` + `persist_links`; `backend/app/models.py:882-952` — `links` table; `frontend/src/components/wiki/NoteView.tsx` — wikilink render | ✅ | | — | — |
...
| ID | Gap | Owner | Effort |
|---|---|---|---|
| G-P1-1 | **LaTeX → Unicode conversion absent.** Add 168-entry mapping to frontend; apply outside math blocks. | [FE] | S |
| G-P1-2 | **KaTeX rendering absent.** Add `remarkMath` + `rehypeKatex` to MarkdownView; auto-wrap bare environments. | [FE] | S |
| G-P1-3 | **Activity panel — per-task status/retry/cancel.** Upgrade `ActivityBar` from 28px strip to a panel with per-task progress + retry/cancel affordance (mirrors llm_wiki `activity-panel.tsx`). | [FE] | M |
...
| G-P1-5 | **Graph insights panel** (surprising connections, knowledge gaps, bridge nodes) — depends on G-P0-2 (Louvain). Add `GraphInsightsPanel` component; dismissable items; click-to-highlight. | [BE]/[FE] | L |
...
| G-P1-7 | **typeAffinity signal — cross-type bonus matrix.** Replace binary `same_type` signal with `TYPE_AFFINITY` matrix that rewards cross-type pairs (entity↔concept 1.2) and penalizes same-type (entity↔entity 0.8). Aligns with llm_wiki code (not README). | [BE] | S |
...
| G-P1-14 | **Clip endpoint body cap.** Ensure `POST /clip` enforces `Content-Length` cap equivalent to llm_wiki `MAX_BODY_BYTES`. | [BE] | S |
| G-P1-15 | **i18n key-count parity test.** Add vitest test that asserts EN and IT JSON have same key set (equivalent to `i18n-parity.test.ts` in llm_wiki). | [FE] | S |
| G-P1-16 | **TanStack Virtual on MessageList + NavTree.** Verify both use `@tanstack/react-virtual` (I4 invariant). | [FE] | S |
| G-P1-17 | **Settings persistence.** Confirm `settingsStore` writes to localStorage (or equivalent) and survives browser refresh. | [FE] | S |
| G-P1-18 | **CI test gate.** Ensure pytest + vitest run on every PR (not just build check). | [DevOps] | S |
