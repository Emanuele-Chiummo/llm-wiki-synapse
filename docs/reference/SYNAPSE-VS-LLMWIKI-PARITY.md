...
| Language-aware generation (language directive) | `buildLanguageDirective` injects "MANDATORY OUTPUT LANGUAGE: X" in both steps; `contentMatchesTargetLanguage` soft-checks at write | `backend/app/ingest/schemas.py` — `WikiFrontmatter.lang`; **overview** regen now injects a mandatory language directive (2026-07-01): explicit from `analysis.language` (orchestrated) or `_detect_vault_language()` (delegated — modal `lang` of existing pages), else "match purpose+pages". Page-generation lang directive still relies on provider prompts. | 🟡 | Overview language fixed (was English-defaulting). Remaining: confirm ApiProvider/OllamaProvider page-generation prompts inject the same directive + a CJK/Latin output soft-check (G-P1-8). | P1 | [AI] |
