...
## 19b. B1 — Lint UI parity (UI-ALIGNMENT-PLAN-2026-07 §B1, ADR-0058, 2026-07-06)

> llm_wiki reference (`lint-view.tsx` / `lint-fixes.ts`): issue-count badge, `Semantic (LLM)`
> checkbox, severity group headers (`⚠ Warnings (741)`), multi-select batch bar
> (`Select all / Fix selected / Send selected to Review / Ignore selected`), per-row `Open` /
> `Fix`, green `Suggested target:` strip, `Delete` on orphan rows.
> All gaps closed in branch `feat/b1-lint-parity`. ADR-0058 is the governing decision.

| Feature / sub-behavior | llm_wiki behavior | Synapse state (code ref) | Verdict | Notes |
|---|---|---|---|---|
| `broken-wikilink` finding category (L1) | `lint-view.tsx` — "Warnings (741)" are structurally broken wikilinks | `backend/app/ops/lint.py` — `broken-wikilink` added to `_VALID_CATEGORIES`; deterministic phase reads `links.dangling=True` (indexed scan, `links.py`); zero LLM cost (I7); dedup on `(vault_id, referencing_page_id, target_text)`. `severity=warning` matches llm_wiki grouping. | ✅ | **L1 CLOSED (2026-07-06 / B1).** The "741 warnings" gap closed for free — existing DB signal turned into surfaced findings. ADR-0058 §2.1. |
| `suggested_target` green strip + DB columns (L2) | `lint-view.tsx` — green `Suggested target:` strip per broken-wikilink row | `backend/app/models.py` / Alembic migration 0024 — `lint_findings.suggested_target` (Text, nullable) + `lint_findings.suggested_page_id` (UUID FK→pages, nullable). Computed at scan time via tolerant resolver (exact→case-insensitive→slug, same as `reresolve_dangling_links`). `NULL` when no plausible match. ER regenerated (`docs/er/schema.mmd`). `frontend/src/components/lint/LintView.tsx` — green strip renders `suggested_target` when present. | ✅ | **L2 CLOSED (2026-07-06 / B1).** Two new nullable columns, migration 0024. ADR-0058 §2.2 + §3. |
| `Fix` for broken-wikilink rewrite (L3) | `lint-fixes.ts` — fix rewrites the dangling `[[target]]` to the suggested title | `backend/app/ops/lint.py:apply_lint_fix` — new `broken-wikilink` branch: when `suggested_target != NULL` rewrites `[[dangling]]` → `[[suggested_target]]` (alias-preserving, frontmatter-preserving round-trip, I5); re-persists links for the referencing page (`dangling→False`); one `data_version` bump (I1). When `suggested_target == NULL` → flag-only (no file write). Same safety class as `missing-xref`. | ✅ | **L3 CLOSED (2026-07-06 / B1).** One link, one page, one bump. ADR-0058 §2.2. |
| Per-row `Open` (navigate to page) (L4) | `lint-view.tsx` — `Open` button navigates to the referencing page | `frontend/src/components/lint/LintView.tsx` — per-row `Open` button wired to `selectPage(target_page_id, "tree")` (`graphStore.ts:296` — existing seam). For `broken-wikilink`, navigates to the **referencing** page (the page containing the broken link). | ✅ | **L4 CLOSED (2026-07-06 / B1).** Wire-only; no new backend surface. ADR-0058 §2.6. |
| Multi-select batch bar (L5) | `lint-view.tsx` — `Select all` + `Fix selected / Ignore selected / Send selected to Review` batch bar | `frontend/src/store/lintStore.ts` — `selectedFindings: Set<id>`; `frontend/src/components/lint/LintView.tsx` — checkboxes, `Select all`, batch bar. `backend/app/routers/lint.py` — `POST /lint/findings/batch` `{ids: UUID[], action: "apply"|"dismiss"|"send-to-review"}`; cap `ids ≤ 200` (>200 → 422, I7); sequential server-side; per-id result list. | ✅ | **L5 CLOSED (2026-07-06 / B1).** New endpoint `POST /lint/findings/batch`. ADR-0058 §2.3. |
| Lint→Review bridge (L6) | `lint-view.tsx` — "Send selected to Review" feeds the review queue | `backend/app/routers/lint.py` — `POST /lint/findings/{id}/send-to-review` calls `ops/review.enqueue_review()` with category→item_type mapping (`broken-wikilink`→`missing-page`, `contradiction`→`contradiction`, etc.); FNV-1a idempotent (re-bridge = NO-OP); finding `status→applied` + `resolution_note` carrying `review_item_id`. | ✅ | **L6 CLOSED (2026-07-06 / B1).** Reuses existing `enqueue_review` seam. ADR-0058 §2.3 + §2.4. |
| Severity group headers (L7) | `lint-view.tsx` — sticky `⚠ Warnings (741)` / `Errors (N)` / `Info (N)` group headers | `frontend/src/components/lint/LintView.tsx` — severity group headers as **synthetic rows** inside the existing TanStack-virtualised list (I4); no de-virtualisation; groups `Errors / Warnings / Info` with per-group counts. | ✅ | **L7 CLOSED (2026-07-06 / B1).** Synthetic-row approach preserves I4. ADR-0058 §2.6. |
| `Semantic (LLM)` toggle + `?semantic=false` path (L8) | `lint-view.tsx` — `Semantic (LLM)` checkbox; unchecked = free deterministic-only scan | `backend/app/routers/lint.py` — `POST /lint/scan?semantic=bool` (default `true`); `false` → skip provider pass entirely (I6/I7). `frontend/src/components/lint/LintView.tsx` — checkbox persisted in `settingsStore`; disabled+tooltip when no provider configured. | ✅ | **L8 CLOSED (2026-07-06 / B1).** `?semantic=false` runs only deterministic phase (orphan-page + broken-wikilink). Zero provider cost. ADR-0058 §2.3. |
| `Delete` on orphan pages (L9) | `lint-view.tsx` — `Delete` button on orphan rows; two-stage confirm | `backend/app/routers/pages.py` — `DELETE /pages/{id}`: scope guard (wiki/ only; index/log/overview → 409); soft-delete row (`deleted_at`); Qdrant point removed; file deleted; index.md regenerated; dead wikilinks → plain text (reuse cascade-delete seam, I5); log.md appended; one `data_version` bump (I1). **UI-only, human-double-confirmed; never callable from any loop** (K8, ADR-0058 Do-NOT #11). | ✅ | **L9 CLOSED (2026-07-06 / B1).** Reuses cascade-delete cleanup seams. K8 preserved end-to-end. ADR-0058 §2.5. |
| Category/severity filters (L10) | `lint-view.tsx` — category and severity filter chips | `backend/app/routers/lint.py` — `GET /lint/findings?category=&severity=` — enum-validated filters (values from `_VALID_CATEGORIES` / `_VALID_SEVERITIES`; invalid → 422); composes with existing `status` filter and cap-200 pagination. `frontend/src/components/lint/LintView.tsx` — filter chips in header. | ✅ | **L10 CLOSED (2026-07-06 / B1).** Enum-validated query params. ADR-0058 §2.3. |

**Invariants:** I1 (single-page edits, one bump) · I4 (virtualised headers, synthetic rows) · I5 (body-only wikilink writes) · I6 (semantic pass provider-neutral; `?semantic=false` resolves no provider) · I7 (batch cap ≤ 200; broken-wikilink is zero-cost; `?semantic=false` is free) · K8 (delete = human double-confirm; no automated path).
**ADR:** ADR-0058 (accepted 2026-07-06) — governs this entire section.
