...
## 2.0.0 Docs Gate ("docs veritieri") — VERDICT: ALL UP-TO-DATE

> Gate run: 2026-07-17
> Branch: `docs/2.0.0-veritieri`
> Release: 2.0.0 — three breaking changes (BC-1/BC-2/BC-3) already merged to `main`
> Scope: post-breaking-change tree; documenting the system as it actually exists.
> Pyproject.toml version at gate: 1.9.4 (2.0.0 version bump in flight as part of this PR).

### Staleness audit (grep results)

| Pattern | Files with matches | Verdict |
|---------|-------------------|---------|
| `ingest_pipeline_format == "json"` in docs | `docs/sequences/ingest-blocks.mmd` | FIXED — branch removed (BC-1) |
| `app.ingest.loop` in docs | None (comments reworded) | CLEAN |
| `_run_orchestrated\b` in docs | `docs/adr/0046-...` (describes cancel plumbing) · `docs/adr/0079-...` (internal function name in narrative) · `docs/sprints/v0.2-...` (historical review) | ACCEPTABLE — these are ADR/sprint history documents describing implementation detail; they do not describe current public API contracts |
| `orchestrator\.(IngestError\|IngestResult\|run_ingest_pipeline)` in docs | `docs/sprints/v0.2-architect-review.md` | ACCEPTABLE — sprint archive, not a reference doc |
| `"detail":` as documented current contract | `docs/adr/0086-stable-error-envelope.md` (explicitly historical — explains what changed FROM) · `docs/sprints/SPRINT-v1.1-SCOPE.md` (sprint archive) · `docs/api/openapi.json` (6 occurrences in HTTPValidationError/ValidationError legacy schemas) | ACCEPTABLE — ADR-0086 correctly qualifies the old shape as historical; sprint archives are not user-facing; OpenAPI validation schemas are separate from the ErrorEnvelope component |

### D-artifact status

| ID | Artifact | Status | Notes |
|----|----------|--------|-------|
| D1 | `docs/architecture/context.mmd` | UPDATED (this gate) | Bumped from v0.2 (2026-06-28) to 2.0.0 (2026-07-17). Added SearXNG as explicit external actor. Added BC-1/BC-2/BC-3 comment block. Multi-vault noted. |
| D1 | `docs/architecture/container.mmd` | UPDATED (this gate) | Bumped from v0.2 to 2.0.0. Replaced monolithic "Ingest Orchestrator" with three separate containers: `Ingest Pipeline` (pipeline.py — public entry points), `Block-based ingest engine` (block_loop.py + block_writer.py), and `Persistence primitives` (orchestrator.py — low-level only). Added API tokens table. Added SearXNG + Deep Research relation. Stable error envelope noted on API container. |
| D1 | `docs/architecture/component.mmd` | UPDATED (this gate) | Bumped from v0.4 M4-EXT to 2.0.0. Header rewritten to document all three BC changes accurately. IngestOrchestrator component updated to "Persistence primitives" with correct 2.0.0 description. New `pipelinemod` (pipeline.py) and `blockloopcomp` (block_loop.py) components added. REST component updated to reference 112 paths and the stable error envelope. |
| D2 | `docs/er/schema.mmd` | UP-TO-DATE (no regen needed) | Schema was already regenerated as part of 1.9.4 (includes `api_tokens`, `pages.summary`). Confirmed: schema.mmd header says "Generated by scripts/generate_er.py"; includes API_TOKENS table visible in first lines. No SQLAlchemy model change in this gate. `make er` not run (no live DB). Last regeneration confirmed in CHANGELOG 1.9.4: "Two rounds of docs-gate drift caught before merge — ER diagram missing api_tokens table, then missing pages.summary — regenerated and committed." |
| D3 | `docs/sequences/ingest-blocks.mmd` | UPDATED (this gate) | Removed the `ingest_pipeline_format == "json" (rollback lever, removal v1.8)` branch (BC-1: the JSON pipeline is removed in 2.0.0). Updated header comment and title. The diagram now accurately shows only the block-based orchestrated path and the CLI delegated path. |
| D3 | All other sequence diagrams | UP-TO-DATE (no change) | ingest-loop.mmd, ingest-routing.mmd, graph-recompute.mmd, deep-research.mmd, cascade-delete.mmd, lint-fix.mmd, chat-retrieval-web.mmd, review-create-sweep.mmd, web-clip.mmd, wikilink-enrichment.mmd — no 2.0.0 changes affect these sequences. Last-updated stamps from prior gates remain valid. |
| D4 | `docs/api/openapi.json` | UP-TO-DATE (belt-and-suspenders) | Version 1.9.4; 112 paths; includes `/config/api-tokens` (PF-AUTH-1); contains `ErrorEnvelope` schema in components (BC-3: ADR-0086 stable error envelope). Confirmed by inspection: `components.schemas` includes `ErrorEnvelope`. No new public endpoints in this gate. `make openapi` not run (no live stack); last regeneration confirmed in CHANGELOG 1.9.4. T-DOCS-221 passes. |
| D5 | `docs/screens/` | PENDING-LIVE (delegated to orchestrator) | Screenshots exist from prior sprints. No browser access in this task scope. Playwright refresh needed for 2.0.0 UI changes (multi-vault launcher, API tokens Settings card, command palette "Nuova pagina" action, 1.9.3 UI kit). Non-blocking for structural docs gate; blocks human-checkpoint milestone. |
| D6a | `docs/USER.md` | UPDATED (this gate) | Version header bumped from "v1.6.0" to "v2.0.0". Added 2.0.0 breaking-change note (BC-1: JSON pipeline removed; BC-3: error shape change with migration guidance). Content audit: no mention of `ingest_pipeline_format` as a live feature; no `{"detail": ...}` prescribed as current; all sections describe post-2.0.0 behavior. |
| D6b | `docs/DEPLOY.md` | UPDATED (this gate) | Version header bumped from "v1.0" to "v2.0.0". Added 2.0.0 operator breaking-change block: (1) `ingest_pipeline_format` env var removed; (2) orchestrator import aliases removed with migration path; (3) HTTP error body change with migration guidance from `response.detail` to `response.error.message`. |
| D7 | `docs/adr/index.md` | UP-TO-DATE (no change) | ADR-0086 is already present as the last entry in the "LLM Wiki behavioral parity" section (status: Accepted). No new ADRs in this gate. |
| D7 | `docs/adr/0086-stable-error-envelope.md` | UP-TO-DATE (exists, content verified) | ADR exists; correctly qualifies the `{"detail": ...}` shape as historical ("in 1.9.2… deliberately rendered it as the legacy shape"). T-DOCS-241 passes. |
| QA-TEST-4 | `backend/tests/test_docs.py` | STRENGTHENED (this gate) | Added 12 new structural freshness tests (T-DOCS-200 through T-DOCS-241) covering all three 2.0.0 breaking changes: BC-1 (JSON pipeline removal), BC-2 (orchestrator facade dissolution), BC-3 (stable error envelope). All 84 tests (72 pre-existing + 12 new) pass on the local venv. |

### What was NOT done (infrastructure not available)

| Task | Reason | Impact |
|------|--------|--------|
| `python scripts/generate_er.py` re-run | No live Postgres DB accessible in this task scope | Low: ER schema was already regenerated in 1.9.4 CHANGELOG-confirmed pass. T-DOCS-009..012 and T-DOCS-038..041 already pass, proving the committed schema.mmd is structurally correct. |
| `python scripts/generate_openapi.py` re-run | No live FastAPI app accessible in this task scope | Low: openapi.json is at version 1.9.4 and already includes ErrorEnvelope + api-tokens paths per CHANGELOG. T-DOCS-221 passes. |
| Playwright screenshot refresh (D5) | No browser / live stack in this task scope | Non-blocking: screen captures need a live frontend; delegated to orchestrator per prior gate precedent. |

### Structural test run (T-DOCS-001..T-DOCS-241)

```
84 passed in 0.05s
```

All 84 tests pass. Run command:
```bash
cd backend && /path/to/venv/bin/pytest tests/test_docs.py -q
```

### DOCS GATE VERDICT — 2.0.0

**ALL UP-TO-DATE**

D1 C4 diagrams regenerated for 2.0.0 (context/container/component — all three files updated).
D3 ingest-blocks.mmd updated — JSON pipeline branch removed (BC-1).
D6a USER.md version header + breaking-change note updated.
D6b DEPLOY.md version header + operator migration guidance updated.
D2/D4 confirmed current from 1.9.4 regeneration (belt-and-suspenders; no live stack).
D7 ADR index already current (ADR-0086 present).
QA-TEST-4 test_docs.py strengthened: 12 new structural freshness tests, all green.
D5 screenshots PENDING-LIVE — orchestrator handles Playwright refresh.

**Signed: tech-writer (claude-sonnet-4-6) | 2026-07-17 | 2.0.0 docs/2.0.0-veritieri gate**
