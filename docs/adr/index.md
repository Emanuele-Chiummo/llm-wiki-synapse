...
| **0086** | [Stable JSON error envelope across backend, frontend, and MCP (2.0.0 breaking change 3) — every error body becomes `{"error": {code, message, status, details}}`; `code` derived mechanically from `SynapseError` subclass name (strip `Error`, snake_case); three handlers (SynapseError / RequestValidationError / HTTPException fallback) all emit the one envelope; 422s carry field-level `details`; `providerClient.formatDetail` removed; MCP tool errors intentionally unchanged; retires the 1.9.2 byte-for-bit non-regression contract](0086-stable-error-envelope.md) | Accepted |
| **0087** | [Content Security Policy (SEC-CSP-1 / 2.0.0) — `script-src 'self'` + `style-src 'self' 'unsafe-inline'`; `unsafe-inline` for styles required by KaTeX HTML+MathML output (inline span style attrs), React inline style={{}} props (1 765 occurrences), and index.html inline `<style>` block; dev/CI Vite headers broaden `connect-src` for cross-origin localhost; Playwright E2E suite in `e2e/csp.spec.ts` verifies zero violations in both themes across Chat/Search/Graph/Settings/KaTeX](0087-csp-policy.md) | Accepted |
...
| **0090** | [MCP OAuth 2.1 + PKCE authorization server for claude.ai custom connectors — amends ADR-0029 §2.2's OAuth deferral: `/authorize`·`/token`·`/register`·`.well-known/*` (root-level paths, matching claude.ai's observed fallback) gate an OAuth-issued access token behind the SAME static MCP token approving the consent step; JIT client registration + PKCE-only public clients; rotate-on-use refresh; shares the `remote_mcp_enabled` floor with `/mcp/server`](0090-mcp-oauth-authorization-server.md) | Accepted |
